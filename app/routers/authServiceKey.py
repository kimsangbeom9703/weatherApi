...
@auth_service_key_router.get("/list", response_model=List[authServiceKeySchemas.ServiceKeyList],
...
@auth_service_key_router.post("/key", response_model=authServiceKeySchemas.ResponseModel,
                              status_code=200)  # 키 생성
async def create_key(req: authServiceKeySchemas.ServiceKeyCreate, db: Session = Depends(get_db)):
    authServicekeyStatus = AuthServiceKeyManager(db=db).createAuthServiceKey(req)
    return authServicekeyStatus


@auth_service_key_router.put("/key", response_model=authServiceKeySchemas.ResponseModel, status_code=200)  # 키 생성
async def update_key(req: authServiceKeySchemas.ServiceKeyUpdate, db: Session = Depends(get_db)):
    authServicekeyStatus = AuthServiceKeyManager(db=db).updateAuthServiceKey(req)
    return authServicekeyStatus
