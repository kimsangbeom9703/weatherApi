...
def read_station(serviceKeyData: str = Depends(get_api_key), commons: dict = Depends(common_parameters),db: Session = Depends(get_db)):
    serviceKeyLogUpdate(db, serviceKeyData, '/api/airport/station')
    total, _list = incheonAirportApi.getDustStationAllData(db=db, page=commons['skip'],size=commons['limit'])  # apis 호출
    return {
        "status": 'success',
        "total": total,
        "data": _list,
...
def read_dust_all(current_time: str = get_current_time(), serviceKeyData: str = Depends(get_api_key),commons: dict = Depends(common_parameters), db: Session = Depends(get_db)):
    serviceKeyLogUpdate(db, serviceKeyData, '/api/airport/air-quality')
    ksb = incheonAirportApi.getDustStationDataByTime(db,current_time)
    print(ksb)
    return '1'
